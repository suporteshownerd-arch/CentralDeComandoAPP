...
def render_page(loader, lojas):
    ...
    if st.session_state.feed_uploaded_images:
        if len(st.session_state.feed_uploaded_images) == 1:
            st.image(st.session_state.feed_uploaded_images[0], width=50)
            ...
            with col_img:
                st.image(st.session_state.feed_uploaded_images[idx], width=50)
    ...
    if st.session_state.feed_imagens:
        ...
        if len(st.session_state.feed_imagens) == 1:
            st.image(st.session_state.feed_imagens[0], width=50)
            col_ex, _ = st.columns([1, 4])
            with col_ex:
            ...
            with col_img:
                st.image(st.session_state.feed_imagens[idx], width=50)
