...
def render_page(loader, lojas):
    ...
    uploaded_file = st.file_uploader("Enviar imagem do PC", type=['png', 'jpg', 'jpeg', 'gif', 'webp'])
    
    if uploaded_file is not None:
        # Converter para URL temporária
        image_url = st.session_state.get("temp_url", {})
        image_url[uploaded_file.name] = uploaded_file
        st.session_state.temp_url = image_url
        st.session_state.feed_imagens.append(f"data:{uploaded_file.type};base64,")
    
    # Campo para adicionar URL
    nova_url = st.text_input("Ou adicionar URL da imagem:", placeholder="https://...")
    ...
    if st.session_state.feed_imagens:
        if len(st.session_state.feed_imagens) == 1:
            ...
            with col_img:
                ...
                if st.button("🗑️ Excluir", key=f"excluir_img_{idx}"):
    ...
    <style>
        ...
        .comunicado-tag {
            ...
            font-size: 10px; font-family: 'JetBrains Mono', monospace; text-transform: uppercase; margin-top: 12px;
        }
    ...
    for com in [
        ...
        <div class="comunicado-card">
            <div class="comunicado-header">
                ...
                <div><div class="comunicado-title">{com["title"]}</div><div class="comunicado-date">{com["date"]}</div></div>
            </div>
